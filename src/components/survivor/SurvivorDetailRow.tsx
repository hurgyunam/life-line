...
export function SurvivorDetailRow({
    survivor,
    isExpanded,
    ...
    const thisPending = pendingActivities.find(
        (a) => a.survivorId === survivor.id,
    );
    const isSearchingFood = thisPending?.type === 'searchFood';
    const isConsuming = Boolean(
        thisPending?.type === 'consumeResource' && thisPending.consumableKey,
    );
    const displayAction = isSearchingFood
        ? t('action.searching_food')
        : isConsuming && thisPending?.consumableKey
          ? t(getConsumableActionKey(thisPending.consumableKey))
          : t(`action.${survivor.currentAction}`);
                    ...
                    <div className="grid gap-x-4 gap-y-2 sm:grid-cols-2">
                        <div>
                            <span className="font-medium text-gray-700">
                                {t('survivorDetail.name')}:
                            </span>{' '}
                            {survivor.name}
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">
                                {t('survivorDetail.age')}:
                            </span>{' '}
                            {survivor.age}
                        ...
                        <div className="sm:col-span-2">
                            <span className="font-medium text-gray-700">
                                {t('survivorDetail.currentAction')}:
                            </span>{' '}
                            {displayAction}
                        </div>
                    </div>
                    ...
                    <div className="border-t border-gray-200 pt-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                                ...
                                title={
                                    isSearchingFood
                                        ...
                                        : t('survivorDetail.hoursRequired', {
                                              hours: ACTIVITY_BALANCE
                                                  .FOOD_SEARCH.DURATION_HOURS,
                                          })
                                }
                            >
                            ...
                            <button
                                ...
                                onClick={() => {
                                    advanceByMinutes(
                                        ACTIVITY_BALANCE.SURVIVOR_SEARCH
                                            .DURATION_HOURS *
                                            GAME_TIME_CONFIG.MINUTES_PER_HOUR,
                                    );
                                    searchSurvivor();
