...
function canExecuteGuideline(
    ...
    switch (guidelineKey) {
        ...
        case 'hungerThreshold': {
            ...
            const foodResource =
                (values.foodResource as FoodResource) ?? 'wildStrawberry';
            if (survivor.hunger >= NEED_SATISFACTION_TARGET)
                return clearAndNo('eatWildStrawberry');
            const inPhase = satisfyingPhase === 'hungerThreshold';
            const atOrBelowThreshold = survivor.hunger <= threshold;
            if (!inPhase && !atOrBelowThreshold)
                return noMatch('eatWildStrawberry');
            if (foodResource === 'wildStrawberry') {
                return {
                    canExecute:
                        useCampResourceStore
                            .getState()
                            .getQuantity('wildStrawberry') > 0,
        ...
        case 'thirstThreshold': {
            ...
            if (survivor.thirst >= NEED_SATISFACTION_TARGET)
                return clearAndNo('drinkWater');
            ...
            return {
                canExecute:
                    useCampResourceStore.getState().getQuantity('water') > 0,
        ...
        case 'tirednessThreshold': {
            ...
            if (survivor.tiredness >= NEED_SATISFACTION_TARGET)
                return clearAndNo('restWithSleepingBag');
            const inPhase = satisfyingPhase === 'tirednessThreshold';
            const atOrBelowThreshold = survivor.tiredness <= threshold;
            if (!inPhase && !atOrBelowThreshold)
                return noMatch('restWithSleepingBag');
            return {
                canExecute: true,
                activityType: 'restWithSleepingBag',
                shouldClearPhase: false,
            };
        }
        case 'boredomThreshold': {
            ...
            if (survivor.boredom >= NEED_SATISFACTION_TARGET)
                return clearAndNo('restAtPlace');
...
export function getGuidelineActivityForSurvivor(
    ...
    for (const key of order) {
        const result = canExecuteGuideline(
            survivor,
            key,
            settings,
            satisfyingPhase,
        ...
        if (result.canExecute) {
            const reserved = toReservedActivityType(
                result.activityType,
                settings,
            );
            return { activity: reserved, newPhase: key, guidelineKey: key };
        }
    }
